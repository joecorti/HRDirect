...
if [ $# -ne 3 ]; then
	echo "Usage: $0 metaNormal.Indel.vcf[.gz] outputBase L_bin" >&2
	exit 6
fi
...
bash $BIN_DIR/get_Indel_freq-distr_anyPerc.NORM.sh $META $L_BIN > $OUT.meta.norm
# get metaNormal distribution
bash $BIN_DIR/get_Indel_freq-distr_anyPerc.sh    $META $L_BIN > $OUT.meta
...
# plot
bash $BIN_DIR/plot_distribution_comparison.Indel.sh $OUT.norm    $OUT.meta.norm $OUT.subtracted.norm $OUT.mirror.norm "$OUT - Normalized"
bash $BIN_DIR/plot_distribution_comparison.Indel.sh $OUT.raw     $OUT.meta $OUT.subtracted $OUT.mirror "$OUT - NON normalized"
